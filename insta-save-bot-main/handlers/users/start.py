        ...
        if malumot[2] == id:
            if  malumot[3] == 'ru':
                await message.answer(f"Отправьте мне ссылку на Instagram – ролики, посты, медиа\n"
                                     "\n"
                                     f"Администратор - @AxrorbekAbdukarimov\n"
                                     f"Свяжитесь с @AxrorbekAbdukarimov, чтобы создать бота")

            elif  malumot[3] == 'en':
                await message.answer(f"Send me Instagram link - Reels, Posts , Media\n"
                                     f"\n"
                                     f"Admin - @AxrorbekAbdukarimov \n"
                                     f"Contact @AxrorbekAbdukarimov to Create a Bot")
            elif malumot[3] == 'uz':
                await message.answer(f"Menga Instagram havolasini yuboring - Reels, Posts , Media\n"
                                     f"\n"
                                     f"Admin - @AxrorbekAbdukarimov \n"
                                     f"Bot Yaratish uchun @AxrorbekAbdukarimov ga Murojaat Qiling")
